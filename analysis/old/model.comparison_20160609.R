  ...
  for (alpha in c(1E-3, 0.002, 0.005,0.008, 0.01,0.03, 0.05)){ #fix alpha or in other words R shape parameter
    ...
    for (i in c(0, 0.5, 1, 2,3,4, 5)){ 
      ...
      weib.likl<-function(param,y){
        ...
        log_s =  exp(-theta*(y^gamma))
        #m.wei = theta * (lifespan^gamma)
        log_m =log(theta)+ gamma*y
        logl = sum(log_s)  + sum(log_m);
      ...
      weib = optim (c(0.025, 0.01), fn=weib.likl, y=lifespan )
      weib$value
      ...
      gomp.likl <- function (param,y){
        ...
        alpha<-param[2]
          ...
          log_s = (alpha/beta) *(1 - exp(beta* y))
          if( alpha< 0 ) { alpha = 1E-10 }
          log_m = log(alpha) +  beta * y ; 
          logl = sum(log_s)  + sum(log_m);
          
          return(-logl)
        }
        # take log(param) since you take exponential above to avoid NaN values above
        gomp = optim ( c(0.0034, 0.1), fn=gomp.likl, y=lifespan)
