...
for (k in  c(0.05,0.08, 0.1,0.12,0.15,0.17, 0.2, 0.25)){
  ...
  data = results.sub[results.sub[,3]==k, 5]
  ...
  heatmap.2(heat_mat, cellnote=hM,col = cm.colors(256), scale="none", notecol="black",  margins=c(5,10),
            ...
            xlab     = "R parameters",
            ylab     = "noise", main = bquote(paste("R vs. sd dLL.flex at" ~ G==.(k))),par(cex.main=.5),srtCol=315, adjCol = c(0,1),cexRow=0.8,cexCol=0.8)
...
#for (n in c(0)){
  data = results.sub[results.sub[,1]==n, 5]
  ...
  #paste(file = "~/github/model.comparison/plots/heatplot_zero_noise_G",k,".jpeg",sep="") 
  jpeg(paste("plots.flex/",n, ".fixed_noise.jpg", sep=''))
  ...
  heatmap.2(heat_mat, cellnote=hM,col = cm.colors(256), scale="none", notecol="black",  margins=c(5,10),
            dendrogram='none', Rowv=FALSE, Colv=FALSE,trace='none',
            xlab     = "R parameters",
            ylab     = "G parameters", main = bquote(paste("R vs G of dLL.flex at" ~ sd==.(n))),par(cex.main=.5),srtCol=315, adjCol = c(0,1),cexRow=0.8,cexCol=0.8)
...
for (j in c(1E-3, 0.002, 0.003, 0.005,0.008, 0.01,0.03, 0.05) ){
  data = results.sub[results.sub[,2]==j, 5]
  ...
  heatmap.2(heat_mat, cellnote=hM,col = cm.colors(256), scale="none", notecol="black",  margins=c(5,10),
            dendrogram='none', Rowv=FALSE, Colv=FALSE,trace='none',
            xlab     = "G parameters",
            ylab     = "noise", main = bquote(paste("G vs. sd of dLL.flex at" ~ R==.(j))),par(cex.main=.5),srtCol=315, adjCol = c(0,1),cexRow=0.8,cexCol=0.8)
